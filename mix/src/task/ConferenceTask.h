            ...
            for(map<unsigned long, PARTICIPANT>::iterator iter = _participants.begin(); iter != _participants.end(); ++iter)
            {
                ...
                if(NULL != ua)
                {
                    if(Speaker == iter->second.role)
                        std::cout << "participant(Speaker): " << ua->_info.id << ", " << ua->_info.ip << ":" << ua->_info.audio_port << " -> " << ua->_info.bind_port << std::endl;
                    else if(Audience == iter->second.role)
                        std::cout << "participant(Audience): " << ua->_info.id << ", " << ua->_info.ip << ":" << ua->_info.audio_port << " -> " << ua->_info.bind_port << std::endl;
                }
            }
        }
