...
//7 添加成员到会议室
MID conf_room_add_member(CID cid, CODEC codec, int mode, unsigned short remote_port, char* remote_ip, unsigned short local_port)
{
    MID mid;
    
    if(SS_NoErr != SINGLETON(CScheduleServer).add_paiticipant(mid, cid, remote_ip, remote_port, codec, Speaker, local_port)) return -1;
    
    return mid;
}

MID conf_room_add_audience(CID cid, CODEC codec, int mode, unsigned short remote_port, char* remote_ip, unsigned short local_port)
{
    MID mid;
    
    if(SS_NoErr != SINGLETON(CScheduleServer).add_paiticipant(mid, cid, remote_ip, remote_port, codec, Audience, local_port)) return -1;
...
void conf_room_rm_member(MID mid, CID cid)
{
    ...
    SINGLETON(CScheduleServer).remove_ua(mid);
}
...
{
    if(SS_NoErr != SINGLETON(CScheduleServer).update_ua_addr(mid, ip, port)) return -1;
...
{
    if(false == CScheduleServer::check_wav_file(fname)) return 1;
    
    CUserAgent* ua = SINGLETON(CScheduleServer).fetch_ua(mid);
    
    if(NULL == ua) return -1;
    
    ua->config_wav_file(fname, times);
...
int conf_init()
{
    ...
    if(SS_NoErr != SINGLETON(CScheduleServer).start_mixer()) return -1;
...
int conf_uninit()
{
    SINGLETON(CScheduleServer).close_all_conference();
...
//开始本地播放wav文件
int local_start_fileplay(char* fname, int times)
{
    if(false == CScheduleServer::check_wav_file(fname)) return 1;
    
    SINGLETON(CScheduleServer).local_start_fileplay(fname, times);
    
    return 0;
}

int local_stop_fileplay()
{
    SINGLETON(CScheduleServer).local_stop_fileplay();
    
    return 0;
}

int p2p_init(char* remote_ip, unsigned short remote_port, unsigned short local_port)
{
    if(SS_NoErr != SINGLETON(CScheduleServer).start_p2p(remote_ip, remote_port, local_port)) return -1;
...
int p2p_uninit()
{
    SINGLETON(CScheduleServer).shutdown_p2p();
