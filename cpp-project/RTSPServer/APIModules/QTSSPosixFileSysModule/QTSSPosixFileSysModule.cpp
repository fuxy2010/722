...
{
	//fym OSFileSource* theFileSource = NEW OSFileSource(inParams->inPath);

	//fym ¶ÁÈ¡Í³Ò»µÄSDPÎÄ¼þ
#ifdef __linux__
	OSFileSource* theFileSource = (NULL != ::strstr(inParams->inPath, ".sdp")) ? NEW OSFileSource("./unique.sdp") : NEW OSFileSource(inParams->inPath);
#else
	OSFileSource* theFileSource = (NULL != ::strstr(inParams->inPath, ".sdp")) ? NEW OSFileSource("unique.sdp") : NEW OSFileSource(inParams->inPath);
#endif
	...
	//qtss_printf("\nFYM Openfile: %s, length: %ld", inParams->inPath, theLength);//fym
...
QTSS_Error  ReadFile(QTSS_ReadFile_Params* inParams)
{
	...
	//qtss_printf("\nFYM Readfile: %ld, %ld", inParams->inBufLen, inParams->outLenRead);//fym
