...
//OSQueue* sRTSPSourceInfoQueue = NULL;//fym ÎªÊäÈëÊý¾ÝÊ±²éÑ¯ÓÃ
...
static QTSS_Error AddRelaySource(QTSS_RelayStreamData_Params* inParams);//fym
static QTSS_Error RemoveRelaySource(QTSS_RelayStreamData_Params* inParams);//fym
//static QTSS_Error InputStreamData(QTSS_RelayStreamData_Params* inParams);//fym
static RTSPSourceInfo* FindExistingRelaySource(UInt16 uuid);//fym
...
QTSS_Error  QTSSRelayModuleDispatch(QTSS_Role inRole, QTSS_RoleParamPtr inParams)
{
    switch (inRole)
    {
        ...
        case QTSS_Shutdown_Role:
            return Shutdown();
		case QTSS_AddRelaySource_Role://fym
			return AddRelaySource(&inParams->relay_stream_data);
		case QTSS_RemoveRelaySource_Role://fym
			return RemoveRelaySource(&inParams->relay_stream_data);
...
QTSS_Error Register(QTSS_Register_Params* inParams)
{
	...
	(void)QTSS_AddRole(QTSS_AddRelaySource_Role);//fym
	(void)QTSS_AddRole(QTSS_RemoveRelaySource_Role);//fym
...
QTSS_Error RereadPrefs()
{
	...
	//fym Ôö¼Ó×ª·¢Ô´
	//AddRelaySource(1);
	//AddRelaySource("192.168.101.131", 554, "h.mp4", "1.sdp");
	//AddRelaySource("192.168.101.204", 554, "x.mp4", "2.sdp");
...
//fym
RTSPSourceInfo* FindExistingRelaySource(UInt16 uuid)
{
	if(0 > uuid)
		return NULL;
	...
	qtss_sprintf(temp, "src%d.mp4", uuid);
	...
	{       
		...
		//if(true == src_url.Equal(info->GetSourceURL()))//fym if(!::strcmp(info->GetSourceURL(), temp))
		if(!::strncmp(info->GetSourceURL(), temp, strlen(temp)))
			return info;
	}
...
//fym ÒÔuuid´úÌæsrc_port,ÔÝ¶¨Ã¿¸ö×ª·¢Ô´ÓÐÁ½¸östream
QTSS_Error AddRelaySource(QTSS_RelayStreamData_Params* inParams)
{
	if(NULL != FindExistingRelaySource(inParams->in_relay_source_uuid))
		return QTSS_RequestArrived;

	if(0 > inParams->in_relay_source_uuid)
		return QTSS_RequestFailed;

	CHAR* src_url = new CHAR[16];
	::memset(src_url, 0, sizeof(src_url));
	qtss_sprintf(src_url, "src%d.mp4", inParams->in_relay_source_uuid);

	CHAR* dest_url = new CHAR[16];
	::memset(dest_url, 0, sizeof(src_url));
	qtss_sprintf(dest_url, "%d.sdp", inParams->in_relay_source_uuid);
	...
	//fym ¼Ó1000±ÜÃâ³öÏÖ¶Ë¿Ú³åÍ»
	theRTSPInfo->AddRelayDestinations("10.10.10.1", inParams->in_relay_source_uuid + 1000, src_url, dest_url);
	theRTSPInfo->StartSessionCreatorTask(sSessionQueue, sRTSPSourceInfoQueue);//fym ½«×ÔÉí²åÈësRTSPSourceInfoQueue
...
//fym
QTSS_Error RemoveRelaySource(QTSS_RelayStreamData_Params* inParams)
{
	RTSPSourceInfo* info = FindExistingRelaySource(inParams->in_relay_source_uuid);

	if(NULL != info)
	{
		delete info;//Îö¹¹º¯ÊýÖÐ¼´ÒÑ¾­½«×ÔÉí´Ó¶ÓÁÐÖÐÈ¡³ö
		info = NULL;
	}
...
/*
QTSS_Error InputStreamData(QTSS_RelayStreamData_Params* inParams)
{
	if(!(inParams->in_packet_len) || NULL == inParams->in_packet_data || 0 > inParams->in_packet_type)
		return QTSS_NoMoreData;

	RTSPSourceInfo* theSoureInfo = FindExistingRelaySource(inParams->in_relay_source_uuid);

	if(NULL == theSoureInfo)
		return QTSS_RequestFailed;

	RelaySession* theSession = theSoureInfo->GetRelaySession();
	...
	theStream->PushRelayPacket(inParams->in_packet_data, inParams->in_packet_len, (UInt32)ntohl(::inet_addr("10.10.10.1")), inParams->in_relay_source_uuid + 1000);
