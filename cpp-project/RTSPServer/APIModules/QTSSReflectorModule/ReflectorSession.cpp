...
//fym for leak
void ReflectorSession::RemoveStreamMap()
{
	if(NULL != sStreamMap)
	{
		delete sStreamMap;
	...
	//fym fMutex(NULL)//fym
{
	...
	fSourceID.Delete();//fym

	if(inSourceID != NULL)
	{
		fSourceID.Ptr = NEW char[inSourceID->Len + 1];

		memset(fSourceID.Ptr, 0, inSourceID->Len + 1);//fym ::ZeroMemory(fSourceID.Ptr, inSourceID->Len + 1);//fym
...
ReflectorSession::~ReflectorSession()
{
	...
	//fym ½«×ÔÉí´ÓsReflectorSessionÖÐÉ¾³ý
    ...
    delete [] fStreamArray;
	fStreamArray = NULL;//fym
    delete fSourceInfo;
	fSourceInfo = NULL;//fym
    fLocalSDP.Delete();
    fSourceID.Delete();
...
{   
    ...
    //fym Assert((fSourceInfo == NULL) || (inInfo == fSourceInfo));
    fSourceInfo = inInfo;
	...
	//qtss_printf("\n<<<<<<<<< NEW ReflectorStream 1 >>>>>>>>>");//fym
    
    for (UInt32 x = 0; x < fSourceInfo->GetNumStreams(); x++)
    {
		//qtss_printf("\nStreamNum: %d", fSourceInfo->GetNumStreams());//fym
        ...
        if(theStreamRef == NULL)
        {
			...
			//fym
			/*if(true == this->fHasBufferedStreams)
				//qtss_printf("\nBNT: %d %d", x, fSourceInfo->GetNumStreams());
			else
				//qtss_printf("\nBNF: %d %d", x, fSourceInfo->GetNumStreams());*/
            ...
            //fym Assert(theError == QTSS_NoErr);
			if(QTSS_NoErr != theError)//fym
				return theError;
            ...
            //fym Assert(debug == fStreamArray[x]->GetRef());
...
{
	//qtss_printf("\nA1");//fym
    //fym Assert(fSourceInfo->GetNumStreams() > 0);
	if(!(fSourceInfo->GetNumStreams()))//fym
		return;
...
//fym
void    ReflectorSession::StreamStatistic()
{
	qtss_printf("\nStreamMap Len: %d", sStreamMap->GetNumRefsInTable());//fym
	qtss_printf("\nSession %s: %d", this->GetSourcePath()->Ptr, this->GetNumOutputs());

	for (UInt32 y = 0; y < fSourceInfo->GetNumStreams(); y++)
	{
		//fym
		qtss_printf("\nStream%d's packet: %d %d", y, fStreamArray[y]->GetRTPSender()->fPacketQueue.GetLength(), fStreamArray[y]->GetRTCPSender()->fPacketQueue.GetLength());
	}
}
...
void    ReflectorSession::TearDownAllOutputs()
{
	for (UInt32 y = 0; y < fSourceInfo->GetNumStreams(); y++)
	{
		...
		/*
		//fym
		ReflectorSender* theRTPSender = fStreamArray[y]->GetRTPSender();
		if(NULL != theRTPSender)
		{
			OSMutexLocker locker(&(theRTPSender->fMutex));//fym
			while (theRTPSender->fPacketQueue.GetLength() > 0)
			{
				ReflectorPacket* packet = (ReflectorPacket*)theRTPSender->fPacketQueue.DeQueue()->GetEnclosingObject();
		...
		//fym
		ReflectorSender* theRTCPSender = fStreamArray[y]->GetRTCPSender();
		if(NULL != theRTCPSender)
		{
			OSMutexLocker locker(&(theRTCPSender->fMutex));//fym
			while (theRTCPSender->fPacketQueue.GetLength() > 0)
			{
				ReflectorPacket* packet = (ReflectorPacket*)theRTCPSender->fPacketQueue.DeQueue()->GetEnclosingObject();
