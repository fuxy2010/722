...
static StrPtrLen    sSDPSuffix(".sdp");//fym Èç¹û¿Í»§¶Ë´«À´µÄÇëÇóÖÐµÄURL²»ÒÔ.sdp½áÎ²£¬ÔòÇ¿ÐÐÔÚURLÄ©Î²¼ÓÉÏ´Ëºó×º
...
//static OSQueue*     sReflectorSessionQueue = NULL;//fym ÔÝÊ±²»ÓÃ£¬ReflecorSessionµÄÎ¬»¤ÓÉsSessionMap¹ÜÀí

//fym
static OSMutex* sSessionMutexArray[65536];
static UInt16	sSessionStatus[65536];//fym 0-Î´Éú³É, 1-ÒÑÉú³É, 2-¼´½«±»Ïú»Ù

//fym
static OSMutex sSessionMutex;//ReflectorSession´´½¨¼°Ïú»ÙµÄ»¥³âÁ¿
...
static SInt32 InputStreamData(QTSS_RelayStreamData_Params* inParams);//fym
static ReflectorSession* FindExistingReflectorSession(UInt16 uuid);//fym
static UInt16 ResolveUUIDFromPath(char* sdp_path);//fym
static Bool16 CheckSDPPath(char* sdp_path);//fym
...
QTSS_Error  QTSSReflectorModuleDispatch(QTSS_Role inRole, QTSS_RoleParamPtr inParams)
{
	switch (inRole)
    {
        ...
        case QTSS_Shutdown_Role:
            return Shutdown();
        case QTSS_RTSPAuthorize_Role://fym ¸Ã½ÇÉ«ÒÑ×¢Ïú
            return ReflectorAuthorizeRTSPRequest(&inParams->rtspRequestParams);
        case QTSS_Interval_Role:
            return IntervalRole();
		case QTSS_InputStreamData_Role://fym
			return InputStreamData(&inParams->relay_stream_data);
		//case QTSS_RemoveReflectorSession_Role://fym
			//return RemoveReflectorSession(&inParams->relay_stream_data);
   }
    return QTSS_NoErr;
}
...
QTSS_Error Register(QTSS_Register_Params* inParams)
{
    ...
    //fym (void)QTSS_AddRole(QTSS_RTSPAuthorize_Role);
    (void)QTSS_AddRole(QTSS_RereadPrefs_Role);
    //fym (void)QTSS_AddRole(QTSS_RTSPRoute_Role);  

	(void)QTSS_AddRole(QTSS_InputStreamData_Role);//fym
	//(void)QTSS_AddRole(QTSS_RemoveReflectorSession_Role);//fym
...
{
    // Setup module utils
	//qtss_printf("\nQTSSReflectorModule:Initialize");//fym
	...
	::memset(sSessionMutexArray, 0, sizeof(sSessionMutexArray));//fym
	::memset(sSessionStatus, 0, sizeof(sSessionStatus));//fym

	//sReflectorSessionQueue = NEW OSQueue();//fym
...
QTSS_Error RereadPrefs()
{
    ...
    {   Bool16 reportErrors = false;
        ...
        if(reportErrors)
        {
            UInt16 minServerPort = 6080;//fym 6970;
...
QTSS_Error Shutdown()
{
	...
	//fym for leak
	ReflectorSession::RemoveStreamMap();

	//fym for leak
	for(UInt32 x = 0; x < 65536; ++x)
	{
		if(NULL != sSessionMutexArray[x])
		{
			delete sSessionMutexArray[x];
			sSessionMutexArray[x] = NULL;
		}
	}
...
QTSS_Error ProcessRTPData(QTSS_IncomingData_Params* inParams)
{
    ...
    if(NULL == theSession || theErr != QTSS_NoErr) 
        return QTSS_NoErr;
...
QTSS_Error ProcessRTSPRequest(QTSS_StandardRTSP_Params* inParams)
{
	...
	//fym ÎªReflectorSession¼ÓËø
	char* theFullPathStr = NULL;
	QTSS_Error Err = QTSS_GetValueAsString(inParams->inRTSPRequest, qtssRTSPReqLocalPath, 0, &theFullPathStr);
	
	if(QTSS_NoErr != Err)//fym
		return QTSS_RequestFailed;
		
	QTSSCharArrayDeleter theFullPathStrDeleter(theFullPathStr);//fym Îñ±ØºÍQTSS_GetValueAsStringÅä¶ÔÊ¹ÓÃ·ñÔòµ¼ÖÂÄÚ´æÐ¹Â©!!!
		
	//qtss_printf("%s", theFullPathStr);//fym
	if(false == CheckSDPPath(theFullPathStr))//fym
		return QTSS_RequestFailed;
		
	UInt16 uuid  = ResolveUUIDFromPath(theFullPathStr);//fym
	OSMutexLocker locker(sSessionMutexArray[uuid]);//fym

	//qtss_printf("\nMethod: %d", *theMethod);//fym
...
//@win32 sdp_path: \XXX.sdp
//@linux sdp_path: \/XXX.sdp
Bool16 CheckSDPPath(char* sdp_path)//fym sdp_path±ØÐëÊÇ"\0.sdp"¡«"\65535.sdp"
{
	if(NULL == sdp_path)
		return false;
	...
	//fym ±ØÐëÒÔ".sdp"½áÎ²
	char* suffix = ::strstr(sdp_path, ".sdp");
	if(NULL == suffix)
		return false;

#ifdef __linux__
	//fym sdpÎÄ¼þÃû±ØÐëÐ¡ÓÚ65535´óÓÚ0
	int sdp_name_len = suffix - sdp_path -2;
	if(5 < sdp_name_len || 0 >= sdp_name_len)//fym "65535"
		return false;

	//fym sdpÎÄ¼þÃûÔÚ1Î»ÒÔÉÏÊ±²»ÄÜÒÔ"0"¿ªÍ·
	if(1 < sdp_name_len && '0' == *(sdp_path + 2))
		return false;

	//fym sdpÎÄ¼þÃû±ØÐëÊÇ´¿Êý×Ö
	char sdp_name[8];
	::memset(sdp_name, 0, sizeof(sdp_name));

	for(UInt32 i = 0; i < sdp_name_len; i++)
	{
		sdp_name[sdp_name_len - i - 1] = *(sdp_path + sdp_name_len - i + 1);
		if(sdp_name[sdp_name_len - i - 1] < '0' || sdp_name[sdp_name_len - i - 1] > '9')
			return false;
	}
#else//win32
	//fym sdpÎÄ¼þÃû±ØÐëÐ¡ÓÚ65535´óÓÚ0
	int sdp_name_len = suffix - sdp_path -1;
	if(5 < sdp_name_len || 0 >= sdp_name_len)//fym "65535"
		return false;

	//fym sdpÎÄ¼þÃûÔÚ1Î»ÒÔÉÏÊ±²»ÄÜÒÔ"0"¿ªÍ·
	if(1 < sdp_name_len && '0' == *(sdp_path + 1))
		return false;
	...
	for(UInt32 i = 0; i < sdp_name_len; i++)
	{
		sdp_name[sdp_name_len - i - 1] = *(sdp_path + sdp_name_len - i);
		if(sdp_name[sdp_name_len - i - 1] < '0' || sdp_name[sdp_name_len - i - 1] > '9')
			return false;
	}
#endif

	//fym sdpÎÄ¼þÃûÖÐµÄÊý×Ö±ØÐëÐ¡ÓÚ65535
	//qtss_printf("\n%s", sdp_name);//fym
	if(65535 < ::atoi(sdp_name))
		return false;
...
{
	//qtss_printf("\nQTSSReflectorModule::DoSessionSetup");//fym
    char* theFullPathStr = NULL;
    QTSS_Error theErr = QTSS_GetValueAsString(inParams->inRTSPRequest, qtssRTSPReqLocalPath, 0, &theFullPathStr);
    //fym Assert(theErr == QTSS_NoErr);
	if(QTSS_NoErr != theErr)//fym
		return NULL;

	/*fym ¸ÄÔÚProcessRTSPRequestÖÐ½øÐÐ
	//fym
	if(false == CheckSDPPath(theFullPathStr))
		return NULL;*/
...
{
	//qtss_printf("\nQTSSReflectorModule::DoAnnounceAddRequiredSDPLines");//fym
    /*fym
...
{
	//qtss_printf("\nQTSSReflectorModule::DoAnnounce");//fym
    ...
    /*fym if(theErr == QTSS_RequestFailed)
    {
	...
	char* theRequestBody = NULL;
	theRequestBody = NEW char[sizeof(temp_sdp) + 1];
	::memset(theRequestBody, 0, sizeof(temp_sdp) + 1);
	::memcpy(theRequestBody, temp_sdp, sizeof(temp_sdp));
...
// ------------  Clean up missing required SDP lines
    ...
    //fym ÒÔÏÂÓï¾ä¼´¿ÉÌæ»»¸Ãº¯ÊýÖ´ÐÐ DoAnnounceAddRequiredSDPLines(inParams, &editedSDP, theRequestBody);
	editedSDP.Put(theRequestBody);

    StrPtrLen editedSDPSPL(editedSDP.GetBufPtr(),editedSDP.GetBytesWritten());
	//editedSDPSPL.PrintStr();//fym
...
{
	//qtss_printf("\nQTSSReflectorModule::DoDescribeAddRequiredSDPLines");//fym
    SDPContainer checkedSDPContainer;
    checkedSDPContainer.SetSDPBuffer( theSDPPtr );  
    if(!checkedSDPContainer.HasReqLines())
    {
...
{
	//qtss_printf("\nQTSSReflectorModule::DoDescribe");//fym
	...
	//qtss_printf("\nDoDescribe: %s", theFilepath);//fym
    
    if(NULL == theSession)//fym
        return QTSS_RequestFailed;
    ...
    //fym Assert(theSession->GetLocalSDP()->Ptr != NULL);
	if(NULL == theSession->GetLocalSDP()->Ptr)//fym
		return QTSS_RequestFailed;
...
//sSessionMapÊÇÒ»¸ö¾²Ì¬µÄÈ«¾Ö±äÁ¿£¬ÕâÀïResolveµÄ²ÎÊýÊÇsdpÎÄ¼þµÄÂ·¾¶, Ò²¾ÍÊÇËµ£¬ÔÚÍ¬Ê±¿ªÆô¶à¸ö´°¿Ú²¥·ÅÍ¬Ò»¸ösdpÎÄ¼þÊ±£¬¶¼ÊÇÊ¹ÓÃÍ¬Ò»¸öReflectorSession¶ÔÏó£¬
//FindOrCreateSessionÒ²²»»áµ÷ÓÃSetupReflectorSessionº¯Êý£¬¼´ReflectorStream¡¢ReflectorSocketµÈ¶ÔÏóÒ²²»»á±»ÔÙ´Î´´½¨¡£
//´ÓÂß¼­ÉÏÀí½âÒ²È·ÊµÈç´Ë£¬ÒòÎªÖ»ÐèÒªÒ»Ì×¶ÔÏóºÍMp4liveÕâÀàÏµÍ³´ò½»µÀ¡£
//µ«ÊÇÔÚÕâÖÖÇé¿öÏÂ£¬»áÖØÐÂ´´½¨RTSPSession¡¢RTPSession¶ÔÏó¡£
//ÕâÑùÔÚQTSSReflectorModule::DoSetupº¯ÊýÀï£¬»áÔÙ´Î´´½¨RTPSessionOutput¶ÔÏó£¬²¢Ìí¼Óµ½ReflectorStreamµÄfOutputArrayÊý×éÀï¡£Í¬Ê±Ò²»áµ÷ÓÃQTSS_AddRTPStreamº¯Êý¡£
...
{   
    ...
    OSRef* theSessionRef = sSessionMap->Resolve(inPath);//fym µÚÒ»´ÎÖ´ÐÐ´ËÓï¾ä£¬ÒòReflectorSessionÎ´´´½¨¹ÊÒýÓÃ¼ÆÊý²»Ôö¼Ó
    ReflectorSession* theSession = NULL;
    ...
    if(theSessionRef == NULL)
    {
		...
		//fym
		UInt16 uuid = ResolveUUIDFromPath(inPath->Ptr);

		if(NULL == sSessionMutexArray[uuid])
		{
			sSessionMutexArray[uuid] = new OSMutex();
		}

		//fym
		if(2 == sSessionStatus[uuid])
			return NULL;
		...
		//sReflectorSessionQueue->EnQueue(theSession->GetQueueElem());//fym
        ...
        if(theErr != QTSS_NoErr)
        {
			OSMutexLocker locker(sSessionMutexArray[uuid]);//fym OSMutexLocker locker(theSession->GetMutex());//fym

			if(2 == sSessionStatus[uuid] || !sSessionStatus[uuid])//fym
				return NULL;

			sSessionStatus[uuid] = 2;//fym

			if(NULL != theSession)//fym
			{
				delete theSession;
				theSession = NULL;//fym
				sSessionStatus[uuid] = 0;//fym
			}
			else//fym
			{
				sSessionStatus[uuid] = 0;//fym
    ...
    else
    {
		...
		sSessionMap->Release(theSessionRef);//fym
...
void DeleteReflectorPushSession(QTSS_StandardRTSP_Params* inParams, ReflectorSession* theSession, Bool16 foundSession)
{
	...
	//fym OSMutexLocker _locker(sSessionMap->GetMutex());//fym

	UInt16 uuid  = ResolveUUIDFromPath(theSession->GetSourcePath()->Ptr);//fym
...
{       
    ...
    //fym Assert(newStreamPtr != NULL);
	if(NULL == newStreamPtr)//fym
		return  QTSS_RequestFailed;
...
{
	//qtss_printf("\nQTSSReflectorModule::DoSetup");//fym
    ReflectorSession* theSession = NULL;
    ...
    //fym Assert(theStreamCookie != NULL);
	if(NULL == theStreamCookie)//fym
		return  QTSS_RequestFailed;
...
QTSS_Error DoPlay(QTSS_StandardRTSP_Params* inParams, ReflectorSession* inSession)
{
	...
	if(NULL == inSession)
		return QTSS_RequestFailed;
	UInt16 uuid  = ResolveUUIDFromPath(inSession->GetSourcePath()->Ptr);//fym
	OSMutexLocker locker(sSessionMutexArray[uuid]);//fym
	if(NULL == inSession)
		return QTSS_RequestFailed;

//#ifdef _DEBUG//fym
	qtss_printf("\nDoPlay %s", inSession->GetSourcePath()->Ptr);//fym
//#endif
    ...
    {   if(!sDefaultBroadcastPushEnabled)
		...
		//fym ±£³ÖResolveºÍReleaseµÄÅä¶Ô
		if(debug != NULL)
		{
			sSessionMap->Release(debug);
			qtss_printf("S3 ");//fym
		}
...
void RemoveOutput(ReflectorOutput* inOutput, ReflectorSession* inSession, Bool16 killClients)
{
    ...
    if(inSession != NULL)
    {
...
#if 0//fym ÐÞ¸ÄÏú»ÙReflectorSessionµÄÄ£Ê½
        ...
        if(theSessionRef != NULL)
        {
			qtss_printf("\n%s count: %d", inSession->GetSourcePath()->Ptr, theSessionRef->GetRefCount());//fym
            ...
            if(theSessionRef->GetRefCount() == 0)
            {
				...
				OSMutexLocker locker(inSession->GetMutex());//fym
				delete inSession;
				inSession = NULL;//fym
            }
...
#else//fym ÐÞ¸ÄÏú»ÙReflectorSessionµÄÄ£Ê½

#ifdef _DEBUG//fym
		qtss_printf("\n\nSessionMap Len: %d", sSessionMap->GetNumRefsInTable());//fym
		inSession->StreamStatistic();
#endif

		if(!(inSession->GetNumOutputs()))//fym if(0)//fym ²»µÃÒÑÒÔÓÀ²»Ïú»ÙReflectorSessionÎª´ú¼Û»»È¡ÎÈ¶¨ÐÔ! (inSession->GetNumOutputs()))
		{
			//OSMutexLocker locker(sSessionMap->GetMutex());//fym

			if(NULL == inSession)
				return;

			//fymÌáÇ° UInt16 uuid  = ResolveUUIDFromPath(inSession->GetSourcePath()->Ptr);//fym

			//fymÌáÇ° OSMutexLocker locker(sSessionMutexArray[uuid]);
			...
			//OSMutexLocker locker(sSessionMap->GetMutex());
#ifdef _DEBUG//fym
			qtss_printf("\nClose %s count: %d---------", inSession->GetSourcePath()->Ptr, inSession->GetRef()->GetRefCount());//fym
#endif

			sSessionMap->Remove(inSession->GetRef());
			sSessionMap->UnRegister(inSession->GetRef());

			if(NULL != inSession)//fym È·±£Èç¹ûÓÐÆäËûÏß³ÌÉ¾³ýÁË´ËSession²»»áÒýÆðÒì³£
			{
				//OSMutexLocker locker(sSessionMutexArray[ResolveUUIDFromPath(inSession->GetSourcePath()->Ptr)]);//fym OSMutexLocker locker(inSession->GetMutex());
				inSession->TearDownAllOutputs();
				delete inSession;
				inSession = NULL;//fym

				OSThread::Sleep(10);//fym
...
#endif//fym ÐÞ¸ÄÏú»ÙReflectorSessionµÄÄ£Ê½
    }
...
//fym
ReflectorSession* FindExistingReflectorSession(UInt16 uuid)
{
	if(0 > uuid)
		return NULL;

	char temp[24];
	::memset(temp, 0, sizeof(temp));//fym ::ZeroMemory(temp, sizeof(temp));
#ifdef __linux__
	qtss_sprintf(temp, "\\/%d.sdp", uuid);//fym Îñ±Ø¼Ó"\\"£¬·ñÔòÓÐÖØÃûsdpÎÄ¼þ qtss_sprintf(temp, "SDP\\%d.sdp", uuid);
#else
	qtss_sprintf(temp, "\\%d.sdp", uuid);//fym Îñ±Ø¼Ó"\\"£¬·ñÔòÓÐÖØÃûsdpÎÄ¼þ qtss_sprintf(temp, "SDP\\%d.sdp", uuid);
#endif

	StrPtrLen src_url(temp);

	//OSMutexLocker locker(sSessionMap->GetMutex());
	OSRef* theSessionRef = sSessionMap->Resolve(&src_url);
	ReflectorSession* theSession = NULL;

	if(NULL != theSessionRef)
	{
		theSession = (ReflectorSession*)theSessionRef->GetObject(); 
		sSessionMap->Release(theSessionRef);
	}
...
//ÊäÈë´ý·¢ËÍÊý¾Ý
SInt32 InputStreamData(QTSS_RelayStreamData_Params* inParams)
{
	if(NULL == inParams->in_packet_data || !(inParams->in_packet_len) || 0 > inParams->in_relay_source_uuid)
		return 0;

	if(2 == sSessionStatus[inParams->in_relay_source_uuid])//fym
		return 0;

	OSMutexLocker locker(sSessionMutexArray[inParams->in_relay_source_uuid]);//fym OSMutexLocker locker(theSession->GetMutex());//fym

	ReflectorSession* theSession = FindExistingReflectorSession(inParams->in_relay_source_uuid);

	//if(NULL  == theSession)
	if(NULL == theSession)
		return 0;//fym QTSS_RequestFailed;

	//fym Èç¹ûÃ»ÓÐ¿Í»§¶ËÁ¬½ÓÔò²»ÊäÈëÊý¾Ý
	if(!(theSession->GetNumOutputs()))
		return 0;

	if(inParams->in_packet_type >= theSession->GetNumStreams())
		return 0;//fym QTSS_NoMoreData;

	ReflectorStream* theStream = theSession->GetStreamByIndex(inParams->in_packet_type);//0-video, 1-audio

	if(NULL == theStream)
		return 0;//fym QTSS_Unimplemented;

	theStream->PushRelayPacket(inParams->in_packet_data,
		inParams->in_packet_len,
		(UInt32)ntohl(inet_addr("127.0.0.1")),
		10000 + inParams->in_relay_source_uuid,
		inParams->in_packet_type);
		//inParams->in_time_stamp_ssrc);

	return theSession->GetNumOutputs();
}

//fym
/*
QTSS_Error RemoveReflectorSession(QTSS_RelayStreamData_Params* inParams)
{
	ReflectorSession* theSession = FindExistingReflectorSession(inParams->in_relay_source_uuid);
	...
	RemoveOutput(NULL, theSession, true);
...
//@win32 sdp_path: \XXX.sdp
//@linux sdp_path: \/XXX.sdp
UInt16 ResolveUUIDFromPath(char* sdp_path)
{
	if(NULL == sdp_path)
		return 0;

	char uuid[16];
	memset(uuid, 0, sizeof(uuid));//fym ZeroMemory(uuid, sizeof(uuid));

	char* sdp = ::strstr(sdp_path, "\\");

	if(NULL == sdp || NULL == ::strstr(sdp, ".sdp"))
		return 0;
	...
	memcpy(uuid, sdp, ::strstr(sdp, ".sdp") - sdp);

	return (65535 < ::atoi(uuid)) ? 0 : ::atoi(uuid);
}
