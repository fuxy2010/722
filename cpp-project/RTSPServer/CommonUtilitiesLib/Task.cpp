...
//½«ÈÎÎñ×ÔÉíÌí¼Óµ½TaskThreadµÄ´ý´¦ÀíÈÎÎñ¶ÓÁÐfTaskQueueÖÐ
void Task::Signal(EventFlags events)
{
    if(!this->Valid())
        return;

	//qtss_printf("*%d ", events);//fym
    ...
    while (true) 
    {
        ...
        while (!doneProcessingEvent)
        {
            ...
            else
            {
                ...
                if(TASK_DEBUG)
					qtss_printf("TaskThread::Entry run TaskName=%s CurMSec=%.3f thread=%ld task=%ld\n", theTask->fTaskName, OS::StartTimeMilli_Float(), (SInt32) this,(SInt32) theTask);

                theTimeout = theTask->Run();
				//fym qtss_printf("Task: %s\n", theTask->fTaskName);//fym
