...
{
    /*fym sHTTPProxyTunnelMap = new OSRefTable(OSRefTable::kDefaultTableSize);

    // Construct premade HTTP response for HTTP proxy tunnel
	qtss_printf(sHTTPResponseFormatStr, "","","", QTSServerInterface::GetServerHeader().Ptr);//fym
	...
	qtss_printf(sHTTPNoServerResponseFormatStr, "","","","");//fym
  ...
  fCurRTPSession(NULL)//fym
{
    ...
    //fym Assert(fLastRTPSessionIDPtr.Ptr == &fLastRTPSessionID[0]);
	if(fLastRTPSessionIDPtr.Ptr != &fLastRTPSessionID[0])//fym
		return;
...
SInt64 RTSPSession::Run()
{
	...
	//qtss_printf("\n========= RTSPSession::Run 1");//fym
    
    while (this->IsLiveSession())
    {
		//qtss_printf("\nRTSPSession::Run %d", fState);//fym
        ...
        switch (fState)
        {
            case kReadingFirstRequest://13
            {
				//qtss_printf("\nstate: kReadingFirstRequest");//fym
            ...
            case kHTTPFilteringRequest://12
            {
				//qtss_printf("\nstate: kHTTPFilteringRequest");//fym
            ...
            case kWaitingToBindHTTPTunnel://10
				//qtss_printf("\nstate: kWaitingToBindHTTPTunnel");//fym
            ...
            case kSocketHasBeenBoundIntoHTTPTunnel://11
				//qtss_printf("\nstate: kSocketHasBeenBoundIntoHTTPTunnel");//fym
            
                // DMS - Can this execute either? I don't think so... this one
                // we may not need...
                
                // I've been joined, it's time to kill this session.
                //fym Assert(!this->IsLiveSession()); // at least the socket should not report connected any longer
                HTTP_TRACE( "RTSPSession has died of snarfage.\n" )
                break;
                
            
            case kReadingRequest://0
            {
				//qtss_printf("\nstate: kReadingRequest");//fym
                ...
                if((err != QTSS_RequestArrived) && (err != E2BIG) && (err != QTSS_BadArgument))
                {
                    ...
                    if(fOutputSocketP->IsConnected())
                    {
                        ...
                        //fym Assert(fOutputSocketP != fInputSocketP);
                        //fym Assert(!fInputSocketP->IsConnected());
                    ...
                    {
                        //fym Assert(!this->IsLiveSession());
						if(this->IsLiveSession())//fym
							break;

						//fym Ò»µ©¿Í»§¶Ë³öÏÖÒì³£¼´TEARDOWNÏà¹ØµÄRTSPSession
						if(NULL != fCurRTPSession)
						{
							//qtss_printf("<%ul> ", fCurRTPSession);//fym
							//fym fCurRTPSession->Teardown();
						}
            ...
            case kHaveNonTunnelMessage://14
            {
				//qtss_printf("\nstate: kHaveNonTunnelMessage");//fym

                // should only get here when fInputStream has a full message built.
                
                //fym Assert( fInputStream.GetRequestBuffer() );
                
                //fym Assert(fRequest == NULL);
                ...
                //fym Assert(err == QTSS_RequestArrived);
            ...
            case kFilteringRequest://1
            {
				//qtss_printf("\nstate: kFilteringRequest");//fym
            ...
            case kRoutingRequest://2
            {
				//qtss_printf("\nstate: kRoutingRequest");//fym
            ...
            case kAuthenticatingRequest://3
            {
				//qtss_printf("\nstate: kAuthenticatingRequest");//fym
            ...
            case kAuthorizingRequest://4
            {
				//qtss_printf("\nstate: kAuthorizingRequest");//fym
            ...
            case kPreprocessingRequest://5
            {
				//qtss_printf("\nstate: kPreprocessingRequest");//fym
            ...
            case kProcessingRequest://6
            {
				//qtss_printf("\nstate: kProcessingRequest");//fym
            ...
            case kPostProcessingRequest://8
            {
				//qtss_printf("\nstate: kPostProcessingRequest");//fym
...
QTSS_Error RTSPSession::PreFilterForHTTPProxyTunnel()
{
	...
	else if( fHTTPMethod == kHTTPMethodGet )
    {
        ...
        {   // contruct a 200 OK header with an "x-server-ip-address" header
            ...
            //fym Assert( localIPAddr.Len < sizeof( localIPAddrBuf ) );
            ...
            //fym Assert(::strlen(responseHeaderBuf) < kMaxHTTPResponseLen);
    ...
    {
        //fym Assert( theOtherSession->fState == kWaitingToBindHTTPTunnel );
...
OSRef* RTSPSession::RegisterRTSPSessionIntoHTTPProxyTunnelMap(QTSS_RTSPSessionType inSessionType)
{
    ...
    //fym Assert(theRef->GetRefCount() > 0);
...
Bool16 RTSPSession::ParseOptionsResponse()
{
	...
	//fym Assert(fRequest->GetValue(qtssRTSPReqFullRequest)->Ptr != NULL);
	if(NULL == fRequest->GetValue(qtssRTSPReqFullRequest)->Ptr)//fym
		return false;
   ...
   // let's also refresh RTP session timeout so that it's kept alive in sync with the RTSP session.
    ...
    if(fRequest->GetMethod() == qtssOptionsMethod)
    {
        ...
        if( requirePtr && requirePtr->EqualIgnoreCase(RTSPProtocol::GetHeaderString(qtssXRandomDataSizeHeader)) )
        {
            ...
            //fym Assert( bodySizeBytes <= sizeof(RTSPSessionInterface::sOptionsRequestBody) );
    ...
    //fym Assert(fRTPSession != NULL); // At this point, we must have one!
	if(NULL == fRTPSession)//fym
		return;
...
QTSS_Error  RTSPSession::CreateNewRTPSession(OSRefTable* inRefTable)
{
	...
	//fym ÓÉÓÚfRTPSessionÔÚÃ¿´ÎRunºó¶¼±»ÖÃNULL£¬¹ÊÒÔ´ËÊôÐÔ±£´æRTPSessionµØÖ·
	if(NULL != fRTPSession)
		fCurRTPSession = fRTPSession;
    
    {
        ...
        //fym Assert(activationError == QTSS_NoErr);
		if(QTSS_NoErr != activationError)//fym
...
void RTSPSession::SetupClientSessionAttrs()
{
    ...
    //fym Assert(theValue != NULL);
	if(NULL == theValue)//fym
		return;
...
UInt32 RTSPSession::GenerateNewSessionID(char* ioBuffer)
{
    ...
    //fym Assert(::strlen(ioBuffer) < QTSS_MAX_SESSION_ID_LENGTH);
...
#else//fym
#include "QTSServer.h"
//fym unsigned short _latest_sequence = 0;
void RTSPSession::HandleIncomingDataPacket()
{
	return;//fym
	...
	//fym
	/*{
		char* data = fInputStream.GetRequestBuffer()->Ptr;

		int track_id = *(unsigned char*)(data + 1);
		int length = 0x100 * (*(unsigned char*)(data + 2)) + (*(unsigned char*)(data + 3)) + 4;
		unsigned short sequence = 0x100 * (*(unsigned char*)(data + 6)) + (*(unsigned char*)(data + 7));

		if(!track_id)
		{
			//qtss_printf("[%d, %d %d] ", sequence, length, fInputStream.GetRequestBuffer()->Len);

			if(sequence != _latest_sequence + 1)
			{
				qtss_printf("<lost %d, %d> ", _latest_sequence, sequence);
			}
	...
	//´¦ÀíÊÕµ½µÄ·ÇRTSPÇëÇóÊý¾Ý
	//qtss_printf("\nData: %d %s", fInputStream.GetRequestBuffer()->Len - 4, fInputStream.GetRequestBuffer()->Ptr + 4);//fym
	//unsigned long count;
	//::memcpy(&count, fInputStream.GetRequestBuffer()->Ptr + 4, sizeof(unsigned long));
	//qtss_printf("<%d %d> ", fInputStream.GetRequestBuffer()->Len - 4, count);//fym

	if(NULL == QTSServer::fDataCallBackFunc)
		return;

	(*QTSServer::fDataCallBackFunc)(fInputStream.GetRequestBuffer()->Ptr + 4, fInputStream.GetRequestBuffer()->Len - 4);
}
#endif
