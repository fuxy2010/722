...
QTSS_Error RTSPRequestStream::ReadRequest()
{
    ...
    while (true)
    {
		...
		//fRequestPtrÖ¸ÏòÒ»¸öÍêÕûµÄRTSPÇëÇó×Ö·û´®
		...
		//ÈçfRequestPtr²»ÎªNULLÔò»á»°ÖÐÈÔÈ»»¹ÓÐÇëÇó×Ö·û´®£¬±ØÐëÊ×ÏÈ´¦Àí´ËÇëÇó
		//Ê×ÏÈ£¬±ØÐë¼ì²ésocket»º³åÇøÖÐÊÇ·ñ»¹ÓÐ²ÐÓàÊý¾Ý,ÈçÓÐÔò»á»°ÈÏÎªÆäÊÇÒ»¸öÐÂÇëÇóµÄÒ»²¿·Ö
        if(fRequestPtr != NULL)
        {
        ...
        if(newOffset == 0)
        {
            ...
            else
            {
				...
				if(newOffset)
				{
					...
					bool is_rtsp_signal = false;//ÊÇ·ñÎªRTSPÐÅÁî
