...
QTSS_ServerState StartServer(XMLPrefsParser* inPrefsSource, PrefsSource* inMessagesSource, UInt16 inPortOverride, int statsUpdateInterval, QTSS_ServerState inInitialState, Bool16 inDontFork, UInt32 debugLevel, UInt32 debugOptions)
{
    ...
    if(sServer->GetServerState() != qtssFatalErrorState)
    {
		...
		//´´½¨ËùÓÐTimeoutTask¶ÔÏó¹²ÓÃµÄTimeoutTaskThread¶ÔÏó²¢µ÷ÓÃsignal·½·¨½«Æä¼ÓÈëµ½TaskThreadÖÐÔËÐÐ
    ...
    if(sServer->GetServerState() != qtssFatalErrorState)
    {
		...
		//×¢Òâµ÷ÓÃCreateUDPSocketPairº¯Êý´«½øÈ¥µÄPort²ÎÊýÎª0£¬ËùÒÔÔÚ²¥·Å¾²Ì¬¶àÃ½ÌåÎÄ¼þÊ±
...
void RTSP_AddRelaySource(UInt16 uuid)
{
	...
	//¿Ï¶¨Ôö¼ÓÁËQTSS_AddRelaySource_Role½ÇÉ«µÄmoduleÖ»ÓÐÒ»¸ö£¬ËùÒÔÖ±½ÓÓÃ0
...
void RTSP_RemoveRelaySource(UInt16 uuid)
{
	...
	//¿Ï¶¨Ôö¼ÓÁËQTSS_RemoveRelaySource_Role½ÇÉ«µÄmoduleÖ»ÓÐÒ»¸ö£¬ËùÒÔÖ±½ÓÓÃ0
...
{   
	...
	while(1)
	{
		for(unsigned short i = 0; i < num; ++i)
		{
			...
			header->timestamp = htonl(OS::Milliseconds());//fym htonl(timeGetTime());
			header->ssrc = htonl(i);
	...
	memset(&file_stat, 0, sizeof(file_stat));//fym ZeroMemory(&file_stat, sizeof(file_stat));
	...
	while(1)
	{
		...
		memset(content, 0, packet_len + 12);//fym ZeroMemory(content, packet_len + 12);
#else
		//ZeroMemory(content, sizeof(content));
#endif
