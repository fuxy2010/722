...
//#include "QTSSFYMModule.h"//fym
//fym #include "QTSSFileModule.h"
//fym #include "QTSSAccessLogModule.h"
...
//fym #include "QTSSAdminModule.h"
#include "QTSSAccessModule.h"
//fym #include "QTSSMP3StreamingModule.h"
#if MEMORY_DEBUGGING
...
void QTSServer::InitModules(QTSS_ServerState inEndState)
{
	...
	//fym LoadModules(fSrvrPrefs);//¶¯Ì¬¼ÓÔØÄ£¿é£¬¼´Ñ°ÕÒQTSSModulesÄ¿Â¼ÏÂµÄ¶¯Ì¬Á´½Ó¿â²¢¼ÓÔØ
    LoadCompiledInModules();//¾²Ì¬¼ÓÔØÄ£¿é
    this->BuildModuleRoleArrays();
...
Bool16 QTSServer::CreateListeners(Bool16 startListeningNow, QTSServerPrefs* inPrefs, UInt16 inPortOverride)
{
    ...
    for (UInt32 count3 = 0; count3 < theTotalPortTrackers; count3++)
    {
        if(thePortTrackers[count3].fNeedsCreating)
        {
			...
			qtss_printf("%hu", thePortTrackers[count3].fPort);
...
//fym ´ÓÅäÖÃÎÄ¼þstreamingserver.xmlµÄ"rtsp_port"×Ö¶ÎÖÐ»ñÈ¡ÒªÕìÌýµÄRTSP¶Ë¿ÚÊý
...
Bool16  QTSServer::SwitchPersonality()
{
    ...
    /*fym OSCharArrayDeleter runGroupName(fSrvrPrefs->GetRunGroupName());
...
void    QTSServer::LoadCompiledInModules()
{
...
#if 0//fym
	QTSSModule* theFYMModule = new QTSSModule("QTSSFYMModule");
	(void)theFYMModule->SetupModule(&sCallbacks, &QTSSFYMModule_Main);
	(void)AddModule(theFYMModule);
#endif
    ...
    /*fym QTSSModule* theAccessLog = new QTSSModule("QTSSAccessLogModule");
    (void)theAccessLog->SetupModule(&sCallbacks, &QTSSAccessLogModule_Main);
    (void)AddModule(theAccessLog);*/

    /*fym QTSSModule* theFlowControl = new QTSSModule("QTSSFlowControlModule");
...
DataCallBackFunc QTSServer::fDataCallBackFunc = NULL;//fym
...
Task*   RTSPListenerSocket::GetSessionTask(TCPSocket** outSocket)
{
    ...
    // when the server is behind a round robin DNS, the client needs to knwo the IP address of the server
    ...
    *outSocket = theTask->GetSocket();  // out socket is not attached to a unix socket yet.
	...
	//fym ÅÐ¶ÏÊÇ·ñ×î´óÁ¬½ÓÊý
    if(true == this->OverMaxConnections(0))
	{
        this->SlowDown();
	}
    else
...
Bool16 RTSPListenerSocket::OverMaxConnections(UInt32 buffer)
{
    ...
    { 
        maxConns += buffer;
        if((theServer->GetNumRTPSessions() > (UInt32)maxConns) || ( theServer->GetNumRTSPSessions() + theServer->GetNumRTSPHTTPSessions() > (UInt32)maxConns))
        {
			overLimit = true;
        }
    } 
...
void RTPSocketPool::DestructUDPSocketPair(UDPSocketPair* inPair)
{
    ...
    delete inPair;
#else//fym
	UDPSocket* socket = inPair->GetSocketA();
	delete socket;
	socket = NULL;

	socket = inPair->GetSocketB();
	delete socket;
	socket = NULL;
