...
#ifdef __linux__
	sprintf(temp, ":%d/%d.sdp", _rtsp_port, uuid);
#else
...
#else//old version
int CRTSPServer::input_stream_data(unsigned short uuid, void* pData, unsigned long nLen, unsigned short nDataType, unsigned long long llTimeStampSSRC)
#endif
{
...
void CRTSPServer::set_data_callback(void (*pDataCallBackFunc)(const char* data, const unsigned long& length))
{
