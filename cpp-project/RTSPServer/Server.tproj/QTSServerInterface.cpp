...
#else
StrPtrLen               QTSServerInterface::sServerNameStr("RTSPStack");//fym ("DSS");
#endif
...
void QTSServerInterface::KillAllRTPSessions()
{
	...
	for (OSRefHashTableIter theIter(fRTPMap->GetHashTable()); !theIter.IsDone(); theIter.Next())
	{
		...
		//fym theSession->Signal(Task::kKillEvent);
		//fym ¸Ä·¢³¬Ê±ÏûÏ¢£¬±ÜÃâ¹Ø±Õ³ÌÐòÊ±µ¼ÖÂÒì³£,¼ûRTPSession:Run()
		theSession->Signal(Task::kTimeoutEvent);
	} 
}
